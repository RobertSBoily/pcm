...
def get_loot_names(filename):
    # filename is a string indicating the location of the
    # Character Builder file to be scraped.
    tree = etree.parse(filename)
    root = tree.getroot()
    character = root.find('CharacterSheet')

    # Find names of magic items possessed.
    loot_tally = character.find('LootTally')
    loots = list(loot_tally)

    loot_names = []
    for loot in loots:
        if int(loot.get('count')) > 0:
            
            for child in loot:
                if child.get('type') == 'Magic Item':
                    loot_names.append(child.get('name'))

    return loot_names


def get_loot_xml(loot_names):
    loot_xml = []
    
    for child in root:
        if child.get('type') == 'Magic Item':
            for loot in loot_names: # Could I merge this loop with the power loop?
                if child.get('name') == loot: 
                    loot_xml.append(child)

    return loot_xml


def get_loot_display(rules_element):
    # Returns a three-item list. 'Magic Item', Slot, Level.
    # Items with no slot or level will have a blank string for that entry.
    display = ['Magic Item:']
    display.append(get_specific(rules_element, 'Magic Item Type'))
    display.append(get_specific(rules_element, 'Level'))
    return display


def get_loot_rules_text(rules_element):
    # Returns a list of strings.
    # Each string is a line of the rules text.
    # Tabs and newline formatting will be added later.
    rules_text = []
    ...
    for child in rules_element:
        if child.get('name') in ('Item Slot', 'Armor', 'Weapon'): # Start including immediately
            included = True

        if included and child.get('name') and child.text:
            new_line = '<b>%s:</b> %s' % (child.get('name'), child.text)
            new_line = unidecode.unidecode(new_line) # Removes troublesome non-ASCII characters.
            rules_text.append(new_line)
...
def main():
    ...
    loot_names = get_loot_names(character)
    ...
    for loot in loot_names:
        for child in root:
            if child.get('name') == loot and child.get('type') == 'Magic Item':
                loot_xml.append(child)
    ...
    # Write the loot cards.
    for rules_element in loot_xml:
        timestamp = get_timestamp()
        name = rules_element.get('name')

        print('Writing card: ' + name) # Progress reporting
        
        display = get_loot_display(rules_element)
        power_usage = ''
        keywords = ''
        rules_text = get_loot_rules_text(rules_element)
        flavor_text = get_flavor_text(rules_element)
        action_icon, range_icon = get_icons(rules_element)
        attack = abbreviate(get_specific(rules_element, 'Attack'))
        background = 'Orange3'

        output.write('card:\n')
        output.write('\thas styling: true\n')
        output.write('\tstyling data:\n')
        output.write('\t\tborder font color: Black\n')
        output.write('\tnotes: Created with Python\n')
        output.write('\ttime created: %s\n' % (timestamp))
        output.write('\ttime modified: %s\n' % (timestamp))
        output.write('\tname: %s\n' % (name))
